...
export const fetchVehiclesList = async (payload: VehicleListPayload) => {
  ...
  if (payload?.timestamp?.from && payload?.timestamp?.to) {
    queryParams.append('initialTimestamp', payload.timestamp.from.toISOString());
    queryParams.append('finalTimestamp', payload.timestamp.to.toISOString());
  }
  try {
